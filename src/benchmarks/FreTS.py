...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.quantile_linear = nn.Linear(self.pred_len, self.n_quantile * self.pred_len)
    ...
    def tokenEmb(self, x):
        ...
        x = x.unsqueeze(3)
        # x: [Batch, Channel, Input length, 1]
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        if (
            ...
            dec_out = self.forecast(x_enc)[:, -self.pred_len :, :]
            dec_out = self.quantile_linear(dec_out.permute(0, 2, 1)).reshape(
                -1, self.feature_size, self.pred_len, self.n_quantile
            )
            return dec_out.permute(0,2,1,3)  # [B, L, D, Q]
        else:
