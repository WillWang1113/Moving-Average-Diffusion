...
def prepare_train(model_config, data_config, args, n):
    ...
    data_folder = os.path.join(root_pth, args["data_config"])
    ...
    noise_schedule = get_schedule(ns_name, n_steps, train_dl, check_pth=data_folder)
...
def main(args, n):
    ...
    data_config = yaml.safe_load(open(f'configs/{args["data_config"]}.yaml', "r"))
    data_config = exp_parser(data_config, args)
    # data_fn = getattr(dataset, data_config.pop("data_fn"))

    model_config = yaml.safe_load(open(f'configs/{args["model_config"]}.yaml', "r"))
    model_config = exp_parser(model_config, args)

    # bb_, cn_, df_ = (
    #     model_config["bb_config"].get("name"),
    #     model_config["cn_config"].get("name"),
    #     model_config["diff_config"].pop("name"),
    # )
    # train_dl, val_dl, test_dl, scaler = data_fn(data_config)

    # df = getattr(diffusion, df_)
    # exp_name = (
    #     f"{get_expname_(model_config, bb_, cn_, df_)}_{'t' if args['smoke_test'] else n}"
    # )
    ...
    # with open(os.path.join(root_pth, args["data_config"], "scaler.npy"), "wb") as f:
    #     np.save(f, scaler)
    # torch.save(test_dl, os.path.join(root_pth, args["data_config"], "test_dl.pt"))
    # batch = next(iter(train_dl))
    # seq_length, seq_channels = (
    #     batch["observed_data"].shape[1],
    #     batch["observed_data"].shape[2],
    ...
    # model_config["cn_config"]["future_seq_length"] = future_seq_length
    # ns_name = model_config["diff_config"].pop("noise_schedule")
    # n_steps = target_seq_length - 1 if df_ != "DDPM" else model_config["diff_config"]["T"]
    # noise_schedule = get_schedule(ns_name, n_steps, train_dl, check_pth=root_pth)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--data_fn", type=str, default="mfred")
